...
public class EvaluationFonciereCalcul {
    ...
    public JSONObject calculerEvaluationFonciere(Terrain terrain) {
        ...
        double valeurFonciereTotal = calculerValeurFonciereTotale(terrain);;
        double taxeScolaire = calculerTaxe(valeurFonciereTotal, 0.012);
        double taxeMunicipale = calculerTaxe(valeurFonciereTotal, 0.025);
        JSONArray lotsArray = creerLotsArray(terrain);

        return creerResultatJSON(valeurFonciereTotal, taxeScolaire, taxeMunicipale, lotsArray, terrain);
    }
    ...
    private double calculerValeurParLot(Lot lot, Terrain terrain) {
        ...
        arrondirValeur(valeurLot + valeurDroitsPassage + valeurServices);
        return arrondirValeur(valeurLot + valeurDroitsPassage + valeurServices);
    }

    private double calculerValeurLot(Lot lot, Terrain terrain) {
        double prixM2 = getPrixM2(terrain);
        return arrondirValeur(lot.getSuperficie() * prixM2);
    }
    ...
    private double calculerValeurDroitsPassage(Lot lot, double valeurLot, int typeTerrain) {
        ...
        return arrondirValeur(base - (lot.getNombreDroitsPassage() * pourcentage * valeurLot));
    }
    ...
    private double calculerValeurServices(Lot lot, int typeTerrain) {
        ...
        return arrondirValeur(Math.min(valeurService * totalServices, 5000));
    }
    ...
    private JSONObject creerLotJson(Lot lot, Terrain terrain) {
        ...
        lotObject.put("valeur_par_lot", String.format("%.2f $", arrondirValeur(calculerValeurParLot(lot, terrain))));
        return lotObject;
    }

    private JSONObject creerResultatJSON(double valeurFonciereTotale, double taxeScolaire, double taxeMunicipale,
                                         JSONArray lotsArray, Terrain terrain) {
        ...
        JSONArray observations = observationsSouleves(terrain, result);
        if (!observations.isEmpty()) {
            result.put("observations",observations);
        }
        return result;
    }
   public static JSONArray observationsSouleves (Terrain terrain, JSONObject resultat) {
       Observations voir = new Observations();
       voir.verifierObservations(terrain, resultat);
       JSONArray observationSouleve = new JSONArray();
       if (!(Observations.getObservations().isEmpty())) {
               observationSouleve = JSONArray.fromObject(Observations.getObservations());
       }
     return observationSouleve;
   }
